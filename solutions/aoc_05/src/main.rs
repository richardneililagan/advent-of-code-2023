...
fn solve_b(lines: Vec<String>) -> usize {
    let seeds = lines[0][6..]
        .split_whitespace()
        .map(|n| n.parse::<usize>().unwrap())
        .collect::<Vec<_>>();
    ...
    seeds[..]
        .chunks(2)
        .flat_map(|c| {
            let [start, count] = c else {
                unreachable!("There should have been an even count of numbers.")
            };
...
mod tests {
    ...
    fn it_solves_examples_b() {
        ...
        assert_eq!(solve_b(input_parser::parse(input_text)), 46);
    }
}
